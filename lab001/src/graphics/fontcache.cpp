...
font_ptr FontCache::do_font_loading(
    ...
    if (success == false) {
        printf("Error in load_font: fontconfig couldn't find any font of pointsize %u.\n", pointsize);
        if (degrees != 0) {
...
font_ptr FontCache::get_font_info(uint pointsize, int degrees) {
    if (degrees == 0) {
