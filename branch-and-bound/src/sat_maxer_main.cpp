...
struct Visitor {
	...
	template<typename Vertex, typename StateStack>
	void onExplore(const Vertex& vertex, const StateStack& state_stack) {
		// dout(DL::INFO) << vertex.literal << ' ';
		const auto& new_counts = evaluator.addSetting(vertex.literal.id(), !vertex.literal.inverted());
		(void)new_counts;
		(void)state_stack;
	}
	...
	void onLeave(const Vertex& vertex) {
		const auto& new_counts = evaluator.removeSetting(vertex.literal.id(), !vertex.literal.inverted());
...
void satisfy_maximally(const CNFExpression& expression) {
	...
	for (const auto& lit_id : expression.all_literals()) {
		state_stack.emplace_back(State{
			Literal(false, lit_id),
			-100
		});
	}

	int state_stack_pos = 0;
		while (state_stack_pos >= 0) {
			const auto& state = state_stack[state_stack_pos];
			...
			if (skip_branch || state_stack_pos == (int)state_stack.size() - 1) {
				while (state_stack_pos >= 0) {
					auto& new_current_state = state_stack[state_stack_pos];
					visitor.onLeave(new_current_state.vertex);
					if (new_current_state.parent_lower_bound < best_cost and !new_current_state.vertex.literal.inverted()) {
						new_current_state.vertex.literal = Literal(true, new_current_state.vertex.literal.id());
						break;
					} else {
						state_stack_pos -= 1;
					}
				...
				state_stack_pos += 1;
				state_stack[state_stack_pos].vertex.literal = Literal(false, state_stack[state_stack_pos].vertex.literal.id());
				state_stack[state_stack_pos].parent_lower_bound = cost.lower_bound;
			}
		}
